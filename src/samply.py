...
class MnistCNN():
    ...
    def __init__(self, im_size, num_classes, patch_size=5, conv1_channel=32, conv2_channel=64,
        ...
        self.after_pool_size, self.hidden_size = self.im_size / \
            (2 * 2), hidden_size
        self.learning_rate = learning_rate

        # Initialize Placeholders
        self.x = tf.placeholder(
            tf.float32, shape=[None, self.im_size * self.im_size])
    ...
    def instantiate_weights(self):
        ...
        self.W_fc2 = self.weight_variable(
            [self.hidden_size, self.num_classes], "W_fc2")
